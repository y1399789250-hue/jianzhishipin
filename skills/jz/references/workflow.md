...
- `voice`：旁白，决定镜头时长。
- `music`：背景音乐，音量低于人声。
- `scene sfx`：根据文案语义和画面动作添加的定点音效，写入 `scene.sfxEvents`。
- `layer sfx`：人物入场音效。

先为每个镜头写一张音效节拍表，再改 `src/script.json`：

```text
scene_id | atSec | visual_cue | narration_cue | sfx | volume
```

只在观众能看懂声音来源的位置添加音效，例如纸片落地、资料卡贴上、按钮/提示出现、页面翻动、重点道具轻碰。不要把音效铺满整条视频。
